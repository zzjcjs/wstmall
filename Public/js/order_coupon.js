$(document).ready(function(){
    ...
    $('.btnuser1').click(function(e){
			...
			$('.redPapercon').hide();
                        ...
                        ele.find('strong').text(data.list.money);
                        ...
                        $('#redbag_price').data('price',data.list.money);
                        $('#redbag_price span').text(data.list.money);
                        calPayPrice();
                        }
                })
    ...
    //        _data.warehouse_id = $('#itemType').data('warehouse');
    //        _data.w = $('#itemType').data('warehouse');
    //    }
...
function addressInfo(){
    ...
    var ret = {};
    $.ajax({
        url:"/instant/order/getAddressInfo",
        type:"POST",
        data:{'code':code},
        dataType:"json",
        async:false,
...
function checkCoupon(code,e){
    //v2 add w
    var _data = {code:code, 'w' : $('#placeOrder').data('w')};
    //立即购买
    var itemType = $('#itemType').data('ck');
    if(itemType > 0){
        _data.itemType = itemType;
    ...
    $.ajax({
        url:'/instant/coupon/checkCoupon',
        dataType:'json',
        type:'POST',
        data:_data,
        success:function(data){
            if (data.code<0) {
               ...
               // $('#couponDiv').empty().append('<span class="couponActive" data-code="'+code+'" data-reduce="'+data.value+'">代金券减免 '+data.value+' 元 </span>&nbsp;<a class="delCoupon" href="javascript:;">取消</a>');
                ...
                ele.find('strong').text(data.value.toFixed(2));
                ele.find('strong').attr('code',code);
                ele.show();
            	//$('#coupon_price').data('price',parseInt(data.value));
                $('#getcouponlist strong').text('+点击使用代金券');
            	$('#coupon_price').data('price',data.value.toFixed(2));
                $('#coupon_price span').text(data.value.toFixed(2));
                calPayPrice();
                $('.ajaxcoupon').hide();
            }
...
function getUserRedPaperList(){
	...
	if(a.length >0){
		...
		if(a.is(':hidden')){
			...
			$('#getRedPaperList strong').text('－使用红包');
			$('.redPapercon').show();
		}
		return false;
	}
...
function calPayPrice(){
    ...
    var shipping = $('#ckstep2').data('ck').shippingPay;
...
function save_address() {
	...
	var url = '/instant/order/save_address';
...
function showAddress(){
	...
	if(selectAddress.length > 0) {
        ...
        var area = selectAddress.find('.area').text();
...
function check_shipping(tag) {
	...
	if (tag == "name") {
		...
		var provinceId = $("#span_province").find("option:selected").val();
		var cityId = $("#span_city").find("option:selected").val();
		var areaId = $("#span_area").find("option:selected").val();
...
function load_cities() {
	...
	if (provinceId == null || provinceId == "") {
		$("#span_city").html("<select id=\"ship_city\" name=\"ship_city\"><option selected=\"\" value=\"\">请选择：</option></select>");
		$("#span_area").html("<select id=\"ship_area\" name=\"ship_area\"><option selected=\"\" value=\"\">请选择：</option></select>");
		$("#span_town").html("");
		return;
	}
